...
import { getConfig, setConfig, getSuspectKeywords, addSuspectKeyword, removeSuspectKeyword, getBlServers, addBlServer, removeBlServer, getBlTags, addBlTag, removeBlTag, getAllBlServerIds, addWhitelistedRole, removeWhitelistedRole } from "../bot/guild-config-store.js";
import { computeTrustScore } from "../bot/ai-trust-score.js";
import { getRaidAnalysis } from "../bot/ai-raid-predict.js";
import { getBehavior, getBehaviorRisk, getTopRiskyMembers } from "../bot/ai-behavior-tracker.js";
...
import { getImmuneList, addImmuneEntry, removeImmuneEntry } from "../bot/immune-store.js";
import { getQuarantineList, removeQuarantine, QuarantineEntry } from "../bot/quarantine-store.js";
import { getAssistantConfig, setAssistantConfig } from "../bot/assistant-config.js";
...
router.get("/owner/guilds/:guildId/automod", (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      antiCapsEnabled: cfg.antiCapsEnabled,
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.patch("/owner/guilds/:guildId/automod", (req, res) => {
  ...
  const {
    ...
    antilinkAllowedDomains?: string[]; antiCapsEnabled?: boolean;
  };
  try {
    ...
    if (antiCapsEnabled !== undefined) gPatch["antiCapsEnabled"] = antiCapsEnabled;
...
// ── IA Mod Config ─────────────────────────────────────────────────────────────
router.get("/owner/guilds/:guildId/ai-mod", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  try {
    const cfg = getConfig(guildId);
    res.json({
      aiToxicityEnabled:    cfg.aiToxicityEnabled,
      aiToxicityThreshold:  cfg.aiToxicityThreshold,
      aiToxicityAction:     cfg.aiToxicityAction,
      aiAutoPunishEnabled:  cfg.aiAutoPunishEnabled,
      aiAutoLockEnabled:    cfg.aiAutoLockEnabled,
      aiRaidPredictEnabled: cfg.aiRaidPredictEnabled,
      aiRaidPredictJoins:   cfg.aiRaidPredictJoins,
      aiRaidPredictMinAge:  cfg.aiRaidPredictMinAge,
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.patch("/owner/guilds/:guildId/ai-mod", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const {
    aiToxicityEnabled, aiToxicityThreshold, aiToxicityAction,
    aiAutoPunishEnabled, aiAutoLockEnabled,
    aiRaidPredictEnabled, aiRaidPredictJoins, aiRaidPredictMinAge,
  } = (req.body ?? {}) as {
    ...
    aiRaidPredictEnabled?: boolean; aiRaidPredictJoins?: number;
    aiRaidPredictMinAge?: number;
  };
  try {
    const patch: Record<string, unknown> = {};
    if (aiToxicityEnabled    !== undefined) patch["aiToxicityEnabled"]    = aiToxicityEnabled;
    if (aiToxicityThreshold  !== undefined) patch["aiToxicityThreshold"]  = aiToxicityThreshold;
    if (aiToxicityAction     !== undefined) patch["aiToxicityAction"]     = aiToxicityAction;
    if (aiAutoPunishEnabled  !== undefined) patch["aiAutoPunishEnabled"]  = aiAutoPunishEnabled;
    if (aiAutoLockEnabled    !== undefined) patch["aiAutoLockEnabled"]    = aiAutoLockEnabled;
    if (aiRaidPredictEnabled !== undefined) patch["aiRaidPredictEnabled"] = aiRaidPredictEnabled;
    if (aiRaidPredictJoins   !== undefined) patch["aiRaidPredictJoins"]   = aiRaidPredictJoins;
    if (aiRaidPredictMinAge  !== undefined) patch["aiRaidPredictMinAge"]  = aiRaidPredictMinAge;
    if (Object.keys(patch).length > 0) setConfig(guildId, patch as any);
    const cfg = getConfig(guildId);
    res.json({
      aiToxicityEnabled:    cfg.aiToxicityEnabled,
      aiToxicityThreshold:  cfg.aiToxicityThreshold,
...
router.get("/owner/guilds/:guildId/ai-trust", async (req, res) => {
  const { guildId } = req.params as { guildId: string };
  ...
  try {
    ...
    const analysis  = computeTrustScore(member, guildId);
    const behavior  = getBehavior(guildId, userId);
    const risk      = behavior ? getBehaviorRisk(behavior) : null;
    res.json({ ...analysis, behavior, risk });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.get("/owner/guilds/:guildId/ai-raid-status", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  try {
    const analysis = getRaidAnalysis(guildId);
    res.json(analysis);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

router.get("/owner/guilds/:guildId/ai-risky-members", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  try {
    const limit = Math.min(50, parseInt((req.query as { limit?: string }).limit ?? "20", 10));
    res.json(getTopRiskyMembers(guildId, limit));
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.get("/owner/guilds/:guildId/log-channels", (req, res) => {
  ...
  res.json({
    ...
    ticketLogChannelId: cfg.ticketLogChannelId ?? null,
    supportLogChannelId: cfg.supportLogChannelId ?? null,
  });
});
...
router.patch("/owner/guilds/:guildId/log-channels", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { logChannelId, banLogChannelId, generalLogChannelId, inviteLogChannelId, messageLogChannelId, ticketLogChannelId, supportLogChannelId } = req.body as Record<string, string | null>;
  const cfg = getConfig(guildId);
  setConfig(guildId, {
    ...cfg,
    ...
    ticketLogChannelId: ticketLogChannelId !== undefined ? ticketLogChannelId : cfg.ticketLogChannelId,
    supportLogChannelId: supportLogChannelId !== undefined ? supportLogChannelId : cfg.supportLogChannelId,
  });
  res.json(getConfig(guildId));
});

// ── POST /api/owner/guilds/:guildId/setup-logs ────────────────────────────────
// Crée la catégorie "🛡️ novaguard protect" + 4 salons de logs et sauvegarde les IDs en config.
router.post("/owner/guilds/:guildId/setup-logs", async (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const client = getClient();
  if (!client) { res.status(503).json({ error: "Bot non connecté" }); return; }
  ...
  try {
    ...
    const category = await guild.channels.create({
      name: "🛡️ novaguard protect",
      type: ChannelType.GuildCategory,
    });

    const channelDefs = [
      { name: "📋︱logs-modération", configKey: "logChannelId" },
      { name: "📊︱logs-general",    configKey: "generalLogChannelId" },
      { name: "🎟️︱logs-ticket",    configKey: "ticketLogChannelId" },
      { name: "🎧︱logs-support",    configKey: "supportLogChannelId" },
    ] as const;
    ...
    for (const def of channelDefs) {
      ...
      created[def.configKey] = { id: ch.id, name: ch.name };
    }

    // Sauvegarder les IDs dans la config
    const cfg = getConfig(guildId);
    setConfig(guildId, {
      ...cfg,
      logChannelId: created.logChannelId.id,
      generalLogChannelId: created.generalLogChannelId.id,
      ticketLogChannelId: created.ticketLogChannelId.id,
      supportLogChannelId: created.supportLogChannelId.id,
    });
    ...
    res.status(500).json({ error: "Erreur lors de la création des salons", detail: String(err) });
  }
});
...
// ── GET /api/owner/guilds/:guildId/roles ─────────────────────────────────────
router.get("/owner/guilds/:guildId/roles", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const client = getClient();
  const guild = client?.guilds.cache.get(guildId);
  if (!guild) { res.status(404).json({ error: "Serveur introuvable" }); return; }
...
router.post("/owner/guilds/:guildId/custom-commands", (req, res) => {
  ...
  const body = req.body as {
    ...
    embedFooter?: string; embedThumbnail?: string; embedImage?: string;
  ...
  if (!body.name?.trim()) { res.status(400).json({ error: "name requis" }); return; }
  const actionType = body.actionType ?? "text";
  if (actionType === "text" && !body.response?.trim()) { res.status(400).json({ error: "response requis pour le type texte" }); return; }
  const cmd = {
    name: body.name.trim().toLowerCase(),
    createdBy: payload?.userTag ?? "owner",
    createdAt: new Date().toISOString(),
    actionType,
    ...(actionType === "text" ? { response: body.response!.trim() } : {}),
    ...(actionType === "embed" ? {
      embedTitle: body.embedTitle, embedDescription: body.embedDescription,
      embedColor: body.embedColor, embedFields: body.embedFields,
      embedFooter: body.embedFooter, embedThumbnail: body.embedThumbnail, embedImage: body.embedImage,
    } : {}),
    ...(actionType === "moderation" ? {
      modAction: body.modAction, modTarget: body.modTarget ?? "mentioned",
      modDuration: body.modDuration, modReason: body.modReason,
    } : {}),
    ...(actionType === "role" ? {
      roleAction: body.roleAction, roleId: body.roleId,
      roleTarget: body.roleTarget ?? "author",
    } : {}),
  };
...
router.get("/owner/guilds/:guildId/anti-protection", (req, res) => {
  ...
  res.json({
    ...
    antiRoleModifyEnabled: cfg.antiRoleModifyEnabled,
    antiAuditBypassPerms: cfg.antiAuditBypassPerms,
  });
});
...
router.patch("/owner/guilds/:guildId/anti-protection", (req, res) => {
  ...
  const allowed = [
    ...
    "antiEveryoneEnabled", "antiEveryoneTimeoutSecs", "antiRoleModifyEnabled",
    "antiAuditBypassPerms",
  ] as const;
  const patch: Record<string, unknown> = {};
  for (const key of allowed) {
...
router.get("/owner/guilds/:guildId/role-whitelist", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  res.json({ roleIds: getConfig(guildId).whitelistedRoleIds });
});

router.post("/owner/guilds/:guildId/role-whitelist", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { roleId } = req.body as { roleId?: string };
  if (!roleId || !/^\d{17,20}$/.test(roleId)) {
    res.status(400).json({ error: "roleId invalide" }); return;
  }
  const added = addWhitelistedRole(guildId, roleId);
  if (!added) { res.status(409).json({ error: "Rôle déjà protégé" }); return; }
  res.json({ roleIds: getConfig(guildId).whitelistedRoleIds });
});

router.delete("/owner/guilds/:guildId/role-whitelist/:roleId", (req, res) => {
  const { guildId, roleId } = req.params as { guildId: string; roleId: string };
  const removed = removeWhitelistedRole(guildId, roleId);
  if (!removed) { res.status(404).json({ error: "Rôle non trouvé dans la liste" }); return; }
  res.json({ roleIds: getConfig(guildId).whitelistedRoleIds });
});
...
// ── POST /api/owner/guilds/:guildId/suspect-keywords/scan ─────────────────────
router.post("/owner/guilds/:guildId/suspect-keywords/scan", async (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const client = getClient();
  const guild = client?.guilds.cache.get(guildId);
  if (!guild) { res.status(404).json({ error: "Serveur introuvable (bot hors ligne ?)" }); return; }
  const keywords = getSuspectKeywords(guildId);
  if (keywords.length === 0) { res.json({ scanned: 0, matched: 0 }); return; }
  try {
    ...
    for (const [, member] of members) {
      ...
      const dLower = (member.displayName ?? "").toLowerCase();
      const kw = keywords.find((k) => uLower.includes(k) || gLower.includes(k) || dLower.includes(k));
      if (!kw) continue;
      matched++;
      const matchedIn = uLower.includes(kw) ? "username" : gLower.includes(kw) ? "global name" : "pseudo serveur";
      const accountAgeMs = Date.now() - member.user.createdTimestamp;
      await saveSuspectAccount({
        guildId, guildName: guild.name, userId: member.id, userTag: member.user.tag,
        accountAgeDays: Math.floor(accountAgeMs / 86_400_000),
        hasNoAvatar: !member.user.avatar,
        reasons: [`[SCAN] Nom contient le mot-clé : \`${kw}\` (${matchedIn})`],
        actionTaken: "flagged", securityLevel: getConfig(guildId).securityLevel,
        vpnSuspicion: false, userLocale: null,
      }).catch(() => null);
    }
    res.json({ scanned: members.size, matched });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
});

// ── GET /api/owner/guilds/:guildId/mod-roles ──────────────────────────────────
const MOD_PERMS = [
  { key: "Administrator",   label: "Administrateur",          bit: PermissionsBitField.Flags.Administrator },
  { key: "ManageGuild",     label: "Gérer le serveur",        bit: PermissionsBitField.Flags.ManageGuild },
  { key: "ManageRoles",     label: "Gérer les rôles",         bit: PermissionsBitField.Flags.ManageRoles },
  { key: "ManageChannels",  label: "Gérer les salons",        bit: PermissionsBitField.Flags.ManageChannels },
  { key: "KickMembers",     label: "Expulser des membres",    bit: PermissionsBitField.Flags.KickMembers },
  { key: "BanMembers",      label: "Bannir des membres",      bit: PermissionsBitField.Flags.BanMembers },
  { key: "ManageMessages",  label: "Gérer les messages",      bit: PermissionsBitField.Flags.ManageMessages },
  { key: "MuteMembers",     label: "Mettre en sourdine",      bit: PermissionsBitField.Flags.MuteMembers },
  { key: "DeafenMembers",   label: "Rendre sourd",            bit: PermissionsBitField.Flags.DeafenMembers },
  { key: "MoveMembers",     label: "Déplacer des membres",    bit: PermissionsBitField.Flags.MoveMembers },
  { key: "ModerateMembers", label: "Timeout (modérer)",       bit: PermissionsBitField.Flags.ModerateMembers },
  { key: "ManageNicknames", label: "Gérer les pseudos",       bit: PermissionsBitField.Flags.ManageNicknames },
  { key: "ViewAuditLog",    label: "Voir le journal d'audit", bit: PermissionsBitField.Flags.ViewAuditLog },
] as const;

router.get("/owner/guilds/:guildId/mod-roles", async (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const client = getClient();
  const guild = client?.guilds.cache.get(guildId);
  if (!guild) { res.status(404).json({ error: "Serveur introuvable" }); return; }
  try {
    ...
    const list = roles
      ...
      .map((r) => ({
        ...
        permissions: MOD_PERMS.map((p) => ({ key: p.key, label: p.label, has: r.permissions.has(p.bit) })),
      }))
      .filter((r) => r.permissions.some((p) => p.has))
      .sort((a, b) => b.position - a.position);
    res.json(list);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});

// ── PATCH /api/owner/guilds/:guildId/roles/:roleId/permissions ────────────────
router.patch("/owner/guilds/:guildId/roles/:roleId/permissions", async (req, res) => {
  const { guildId, roleId } = req.params as { guildId: string; roleId: string };
  ...
  if (role.managed || role.id === guild.id) {
    res.status(400).json({ error: "Ce rôle ne peut pas être modifié" }); return;
  }
  let perms = role.permissions;
  for (const key of grant) {
    const p = MOD_PERMS.find((m) => m.key === key);
    if (p) perms = perms.add(p.bit);
  }
  for (const key of revoke) {
    const p = MOD_PERMS.find((m) => m.key === key);
    if (p) perms = perms.remove(p.bit);
  }
  try {
    await role.setPermissions(perms, "Modification permissions — Dashboard Owner");
    const updated = guild.roles.cache.get(roleId)!;
    res.json({
      id: updated.id, name: updated.name,
      permissions: MOD_PERMS.map((p) => ({ key: p.key, label: p.label, has: updated.permissions.has(p.bit) })),
    });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
// ── POST /api/owner/deploy-commands ──────────────────────────────────────────
router.post("/owner/deploy-commands", async (req, res) => {
  try {
    const client = getClient();
    if (!client?.application) { res.status(503).json({ error: "Bot non connecté" }); return; }
    const commandData = commands.map((c) => c.data.toJSON() as ApplicationCommandDataResolvable);
    await client.application.commands.set(commandData);
    res.json({ ok: true, count: commandData.length });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.get("/owner/guilds/:guildId/usercommands-config", (req, res) => {
  const guildId = req.params["guildId"] as string;
  const cfg = getConfig(guildId);
  res.json({
    supportChannelId: cfg.supportChannelId,
    supportPingRoleIds: cfg.supportPingRoleIds,
    roleRequestChannelId: cfg.roleRequestChannelId,
    roleRequestPingRoleIds: cfg.roleRequestPingRoleIds,
  });
});

router.patch("/owner/guilds/:guildId/usercommands-config", (req, res) => {
  const guildId = req.params["guildId"] as string;
  const { supportChannelId, supportPingRoleIds, roleRequestChannelId, roleRequestPingRoleIds } = req.body as {
  ...
  if (supportChannelId !== undefined) patch.supportChannelId = supportChannelId || null;
  if (supportPingRoleIds !== undefined) patch.supportPingRoleIds = supportPingRoleIds;
  if (roleRequestChannelId !== undefined) patch.roleRequestChannelId = roleRequestChannelId || null;
  if (roleRequestPingRoleIds !== undefined) patch.roleRequestPingRoleIds = roleRequestPingRoleIds;
...
// ── Suspect Accounts ──────────────────────────────────────────────────────────
import { getSuspectAccounts, saveSuspectAccount, deleteSuspectAccount, markSuspectVerified, updateSuspectTags, getSuspectsByUserId } from "../bot/suspect-accounts-db.js";
...
router.post("/owner/guilds/:guildId/voice-presence/join", async (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { channelId, selfMute = false, selfDeaf = false } = (req.body ?? {}) as { channelId?: string; selfMute?: boolean; selfDeaf?: boolean };
...
router.post("/owner/guilds/:guildId/voice-presence/announce", async (req, res) => {
  ...
  if (text.trim().length > 1000) { res.status(400).json({ error: "Texte trop long (max 1000 car.)" }); return; }
  ...
  // ── PAS de toggle selfMute — le bot doit rejoindre avec selfMute=false (défaut du panel) ──
...
router.get("/owner/immune-list", (_req, res) => {
  res.json(getImmuneList());
});

router.post("/owner/immune-list", (req, res) => {
  const { userId, label } = req.body as { userId?: string; label?: string };
  if (!userId?.trim()) return void res.status(400).json({ error: "userId requis" });
  const entry = addImmuneEntry(userId.trim(), label?.trim() || userId.trim());
  if (!entry) return void res.status(409).json({ error: "Cet utilisateur est déjà dans la liste permanente" });
  res.json(entry);
});

router.delete("/owner/immune-list/:userId", (req, res) => {
  const { userId } = req.params as { userId: string };
  const removed = removeImmuneEntry(userId);
  if (!removed) return void res.status(403).json({ error: "Entrée permanente ou introuvable" });
  res.json({ ok: true });
});

// ── Whitelist membres (par guild) ─────────────────────────────────────────────

router.get("/owner/guilds/:guildId/whitelist-members", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const cfg = getConfig(guildId);
  res.json({ whitelistedMemberIds: cfg.whitelistedMemberIds ?? [] });
});

router.post("/owner/guilds/:guildId/whitelist-members", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { userId } = req.body as { userId?: string };
  if (!userId?.trim()) return void res.status(400).json({ error: "userId requis" });
  const cfg = getConfig(guildId);
  const list = cfg.whitelistedMemberIds ?? [];
  if (list.includes(userId.trim())) return void res.status(409).json({ error: "Déjà whitelisté" });
  setConfig(guildId, { whitelistedMemberIds: [...list, userId.trim()] });
  res.json({ whitelistedMemberIds: [...list, userId.trim()] });
});

router.delete("/owner/guilds/:guildId/whitelist-members/:userId", (req, res) => {
  const { guildId, userId } = req.params as { guildId: string; userId: string };
  const cfg = getConfig(guildId);
  const list = cfg.whitelistedMemberIds ?? [];
  if (!list.includes(userId)) return void res.status(404).json({ error: "Non trouvé" });
  setConfig(guildId, { whitelistedMemberIds: list.filter(id => id !== userId) });
  res.json({ ok: true });
});
...
router.get("/owner/guilds/:guildId/notif-roles", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const cfg = getConfig(guildId);
  res.json({ notifRoles: cfg.notifRoles, notifPanelChannelId: cfg.notifPanelChannelId });
});

router.patch("/owner/guilds/:guildId/notif-roles", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { notifRoles, notifPanelChannelId } = req.body as {
    notifRoles?: { roleId: string; label: string; emoji: string }[];
    notifPanelChannelId?: string | null;
  };
  if (notifRoles !== undefined) setConfig(guildId, { notifRoles });
  if (notifPanelChannelId !== undefined) setConfig(guildId, { notifPanelChannelId: notifPanelChannelId || null });
  const cfg = getConfig(guildId);
  res.json({ notifRoles: cfg.notifRoles, notifPanelChannelId: cfg.notifPanelChannelId });
});

router.post("/owner/guilds/:guildId/notif-panel", async (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const client = getClient();
  const cfg = getConfig(guildId);
  if (!cfg.notifRoles.length) { res.status(400).json({ error: "Aucun rôle configuré." }); return; }
  const channelId: string | undefined = (req.body as { channelId?: string })?.channelId ?? cfg.notifPanelChannelId ?? undefined;
  if (!channelId) { res.status(400).json({ error: "Aucun salon configuré." }); return; }
  if (!client) { res.status(503).json({ error: "Bot non connecté." }); return; }
  const guild = await client.guilds.fetch(guildId).catch(() => null);
  if (!guild) { res.status(404).json({ error: "Serveur introuvable." }); return; }
  const channel = (guild.channels.cache.get(channelId) ?? await guild.channels.fetch(channelId).catch(() => null)) as TextChannel | null;
  if (!channel || !("send" in channel)) { res.status(404).json({ error: "Salon introuvable ou invalide." }); return; }

  const embed = new EmbedBuilder()
    .setColor(0x6366f1)
    .setTitle("🔔 Rôles de notifications")
    .setDescription(
      "Clique sur un bouton pour **recevoir** ou **retirer** un rôle de notification.\n\n" +
      cfg.notifRoles.map(r => `${r.emoji ? r.emoji + " " : ""}**${r.label}** → <@&${r.roleId}>`).join("\n")
    )
    .setFooter({ text: "Clique à nouveau pour retirer le rôle." })
    .setTimestamp();

  const rows: ActionRowBuilder<ButtonBuilder>[] = [];
  for (let i = 0; i < Math.min(cfg.notifRoles.length, 25); i += 5) {
    const row = new ActionRowBuilder<ButtonBuilder>();
    for (const r of cfg.notifRoles.slice(i, i + 5)) {
      const btn = new ButtonBuilder()
        .setCustomId(`notif_role_${r.roleId}`)
        .setLabel(r.label)
...
router.get("/owner/assistant-config", (_req, res) => {
  res.json(getAssistantConfig());
});

router.patch("/owner/assistant-config", (req, res) => {
  const { enabled } = req.body as { enabled?: boolean };
...
router.get("/owner/guilds/:guildId/activity/message-ranking", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { since, channelId, limit } = req.query as { since?: string; channelId?: string; limit?: string };
  const ranking = getMessageRanking(guildId, {
...
router.get("/owner/guilds/:guildId/activity/voice-ranking", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { since, limit } = req.query as { since?: string; limit?: string };
  const ranking = getVoiceRanking(guildId, {
    since: since ? Number(since) : undefined,
    limit: limit ? Math.min(Number(limit), 200) : 50,
  });
  res.json(ranking);
...
router.get("/owner/guilds/:guildId/activity/member/:userId", (req, res) => {
  const { guildId, userId } = req.params as { guildId: string; userId: string };
  const { since } = req.query as { since?: string };
  const sinceTs = since ? Number(since) : undefined;
  const messages = getMemberMessageStats(guildId, userId, { since: sinceTs });
  const voice = getMemberVoiceStats(guildId, userId, { since: sinceTs });
  res.json({ userId, messages, voice });
});

router.get("/owner/guilds/:guildId/activity/tiers", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  res.json(getActivityTiersConfig(guildId));
});

router.patch("/owner/guilds/:guildId/activity/tiers", (req, res) => {
  const { guildId } = req.params as { guildId: string };
  const { activityTiersEnabled, activityTierPeriodDays, activityTiers } = req.body as {
  ...
  if (activityTiersEnabled !== undefined) setActivityTiersEnabled(guildId, activityTiersEnabled);
  if (activityTierPeriodDays !== undefined) setActivityTierPeriodDays(guildId, Math.max(1, Math.min(365, activityTierPeriodDays)));
  if (activityTiers !== undefined) setActivityTiers(guildId, activityTiers);
  res.json(getActivityTiersConfig(guildId));
});
