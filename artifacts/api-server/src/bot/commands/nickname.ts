...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!member.manageable) return interaction.reply({ content: "Je ne peux pas modifier le surnom de ce membre.", ephemeral: true });
...
export async function executeMessage(message: Message, args: string[]) {
  ...
  if (!message.member.permissions.has(PermissionFlagsBits.ManageNicknames)) {
    await message.reply("❌ Permission insuffisante (ManageNicknames requise)."); return;
  }
  ...
  if (args[0].toLowerCase() === "reset") {
    ...
    if (!resetUserId) { await message.reply("Usage : `&nickname reset @membre`"); return; }
  ...
  if (!member.manageable) { await message.reply("❌ Je ne peux pas modifier le surnom de ce membre."); return; }
