...
import { recordJoin as recordRaidJoin, getRaidAnalysis, raidLevelLabel } from "./ai-raid-predict.js";
import { setOwnerIds } from "./owner-store.js";
import { startTempBanScheduler } from "./tempban-store.js";
import { initTempRoleStore, startTempRoleScheduler } from "./temprole-store.js";
...
import { isImmune } from "./is-immune.js";
import { loadImmuneStore } from "./immune-store.js";
...
import { loadAssistantConfig, getAssistantConfig } from "./assistant-config.js";
import { clearTimeoutLog } from "./timeout-log-store.js";
import { addPendingUntimeout as _uto_add, getPendingUntimeout, removePendingUntimeout } from "./pending-untimeout-store.js";
import { isGloballyBlacklistedWord } from "./global-word-blacklist-store.js";
import { getCounter, incrementCounter, resetCounter, initCounterStore } from "./counter-store.js";
import { recordNickChange, setCachedNick, initNickHistoryStore } from "./nick-history-store.js";
import { isWatched, getWatchlistLogChannel, initWatchlistStore } from "./watchlist-store.js";
...
import { initTicketBlacklistStore, isTicketBlacklisted } from "./ticket-blacklist-store.js";
import { initTagStore } from "./tag-store.js";
import { pingActivity } from "./ticket-inactivity-store.js";
...
export function startBot(): void {
  ...
  // Absorbe les rate limits Discord sans crasher — le client gère le retry automatiquement
  client.rest.on("rateLimited", (info) => {
    logger.warn({
      ...
      global: info.global,
    }, "[REST] Rate limited — retry automatique en cours");
  });
  ...
  client.once(Events.ClientReady, async (readyClient) => {
    ...
    // Sérialise chaque commande individuellement pour identifier les erreurs
    const commandData: ApplicationCommandDataResolvable[] = [];
    for (const cmd of commands) {
      try {
        commandData.push(cmd.data.toJSON() as ApplicationCommandDataResolvable);
      } catch (serErr) {
        logger.error({ err: serErr, name: cmd.data.name }, "Échec sérialisation commande — ignorée");
      }
    }
    logger.info({ total: commands.length, serialized: commandData.length }, "Commandes sérialisées, envoi à Discord…");

    let regStatus: "ok" | "error" = "ok";
    let regError: string | null = null;
    try {
      // Enregistrement global (propagation Discord : ~1h mais souvent instantané)
      await readyClient.application.commands.set(commandData);

      // Nettoie les anciennes commandes par serveur si elles existent
      await Promise.all(
        readyClient.guilds.cache.map(g => g.commands.set([]).catch(() => null)),
      );
      logger.info({ count: commandData.length }, "Commandes slash globales enregistrées avec succès");
    } catch (err: unknown) {
      regStatus = "error";
      const discordErr = err as { status?: number; message?: string; rawError?: unknown };
      regError = `HTTP ${discordErr.status ?? "?"} — ${discordErr.message ?? "erreur inconnue"} | rawError: ${JSON.stringify(discordErr.rawError ?? null)}`;
      logger.error({
        status: discordErr.status,
        message: discordErr.message,
        rawError: discordErr.rawError,
      }, "Erreur lors de l'enregistrement des commandes slash");
    }

    await sendStartupAlert(readyClient, commandData.length, prefixCommands.length, regStatus, regError).catch((err) => {
    ...
    startTempRoleScheduler(readyClient);
    await loadImmuneStore().catch(() => null);
    await loadAssistantConfig().catch(() => null);
    await initCounterStore().catch(() => null);
    await initNickHistoryStore().catch(() => null);
    await initWatchlistStore().catch(() => null);
    ...
    await initTicketBlacklistStore().catch(() => null);
    await initTagStore().catch(() => null);
    await initTempRoleStore().catch(() => null);

    // Peuple le cache des pseudos connus pour tous les membres déjà en serveur
    for (const [, guild] of readyClient.guilds.cache) {
      for (const [, member] of guild.members.cache) {
        setCachedNick(guild.id, member.id, member.nickname);
      }
    }
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    // === INACTIVITÉ TICKETS ===
    if (!message.channel.isDMBased() && message.guildId && isTicketChannel(message.channelId)) {
      pingActivity(message.channelId);
    }

    // === COMPTEUR INFINI ===
    if (!message.channel.isDMBased() && message.guildId) {
      const counterCfg = getCounter(message.guildId);
      if (counterCfg.channelId && message.channelId === counterCfg.channelId) {
        const raw = message.content.trim();
        const num = parseInt(raw, 10);
        const isPlainNumber = !isNaN(num) && String(num) === raw;
        if (isPlainNumber) {
          await message.delete().catch(() => null);
          const expected = counterCfg.current + 1;
          if (num === expected) {
            incrementCounter(message.guildId);
            const phrase = COUNTER_PHRASES[num % COUNTER_PHRASES.length]!;
            const embed = new EmbedBuilder()
              .setColor(0x22c55e)
              .setDescription(`<@${message.author.id}> — **${num}** ${phrase}`);
            await message.channel.send({ embeds: [embed] }).catch(() => null);
          } else {
            const reached = counterCfg.current;
            resetCounter(message.guildId);
            const embed = new EmbedBuilder()
              .setColor(0xef4444)
              .setTitle("❌ Mauvais chiffre !")
              .setDescription(
                `<@${message.author.id}> a écrit **${num}** mais il fallait **${expected}** !\n` +
                `Le compteur repart de **${counterCfg.startFrom}**.`
              )
              .setFooter({ text: `Record atteint : ${reached}` });
            await message.channel.send({ embeds: [embed] }).catch(() => null);
          }
        }
        return;
    ...
    // === LAST SEEN + WATCHLIST + ACTIVITÉ ===
    if (!message.channel.isDMBased() && message.guildId) {
      recordSeen(message.guildId, message.author.id);
      recordMessage(message.guildId, message.author.id, message.channelId);
      if (isWatched(message.guildId, message.author.id)) {
        const logChannelId = getWatchlistLogChannel(message.guildId);
        if (logChannelId) {
          const logChan = client.channels.cache.get(logChannelId) as TextChannel | undefined;
          if (logChan) {
            const wEmbed = new EmbedBuilder()
              .setColor(0xf97316)
              .setTitle("👁️ Message — membre surveillé")
              .setDescription(message.content.slice(0, 1024) || "*[pas de texte]*")
              .addFields(
                { name: "Membre", value: `<@${message.author.id}> (\`${message.author.tag}\`)`, inline: true },
                { name: "Salon", value: `<#${message.channelId}>`, inline: true },
              )
              .setTimestamp();
            logChan.send({ embeds: [wEmbed] }).catch(() => null);
          }
        }
      }
    }
    ...
    if (!message.channel.isDMBased() && message.guildId && message.member) {
      ...
      if (spamCfg.antiSpamEnabled) {
        ...
        if (timestamps.length >= spamCfg.antiSpamMessages && !isImmune(message.author.id) && !(getConfig(guildId).whitelistedMemberIds ?? []).includes(message.author.id)) {
          spamMap.delete(key);
    ...
    // === ASSISTANT : réponse aux mentions (@bot) ===
    if (!message.author.bot && !message.channel.isDMBased() && client.user && message.mentions.has(client.user)) {
      const cfg = getAssistantConfig();
      if (cfg.enabled) {
        const txt = message.content.replace(/<@!?\d+>/g, "").trim().toLowerCase();
        const BLAGUES = [
        ...
        const HISTOIRES = [
          "Un chasseur rentre bredouille. Sa femme : « Tu as vu 40 sangliers ?! » Lui : « J'ai pas mon fusil. » Elle : « Comment tu sais qu'il y en avait 40 ? » « Je les ai comptés en courant ! » 🏃",
          "Un patient : « Docteur, je pense que je suis une poule. » Le médecin : « Depuis quand ? » « Depuis l'œuf. » 🐔",
          "Un parachutiste tire sa poignée. Rien. La seconde. Rien. Il voit un gars monter. « Tu connais les parachutes ? » L'autre : « Non, tu connais les gaz ? » 😱",
        ];
        const GREETINGS = [
          "Bonjour ! 👋 Je suis là pour vous aider !",
          "Salut ! 🤖 Comment puis-je vous aider ?",
          "Présent ! ✨ Que puis-je faire pour vous ?",
          "Bonjour, je vous écoute ! 🎧",
        ];
        let reply: string;
        if (txt.includes("bonjour") || txt.includes("salut") || txt.includes("hello") || txt.includes("hi")) {
          reply = `👋 Bonjour <@${message.author.id}> ! Comment puis-je t'aider ?`;
        } else if (txt.includes("aide") || txt.includes("help") || txt.includes("commande")) {
          reply = "💡 Pour la modération, utilise `/commandlist`.";
        } else {
          reply = GREETINGS[Math.floor(Math.random() * GREETINGS.length)]!;
        }
        await message.reply({ content: reply }).catch(() => null);
      }
    }
  ...
  // ──── GUILD ROLE DELETE (protection whitelist) ────
  client.on(Events.GuildRoleDelete, async (role) => {
    const cfg = getConfig(role.guild.id);
    if (!cfg.whitelistedRoleIds.includes(role.id)) return;
    ...
    try {
      const newRole = await role.guild.roles.create({
        ...
        reason: "🔒 Rôle protégé — recréation automatique par NovaNGuard",
      });
      removeWhitelistedRole(role.guild.id, role.id);
      addWhitelistedRole(role.guild.id, newRole.id);
      logger.info({ guild: role.guild.name, oldId: role.id, newId: newRole.id }, "[role-whitelist] Rôle protégé recréé");
      await sendLog(
        client,
        logEmbed(0xfbbf24, "🔒 Rôle protégé recréé automatiquement", [
          { name: "Rôle supprimé", value: `**${role.name}** (\`${role.id}\`)`, inline: true },
          { name: "Nouveau rôle", value: `<@&${newRole.id}> (\`${newRole.id}\`)`, inline: true },
        ], { tag: "NovaNGuard System", id: client.user?.id ?? "0" }),
        { guildId: role.guild.id },
      );
    } catch (err) {
      logger.error({ err, roleId: role.id }, "[role-whitelist] Impossible de recréer le rôle protégé");
    }
  });

  // ──── GUILD MEMBER ADD ────
  client.on(Events.GuildMemberAdd, async (member) => {
    // Les bots sont immunisés de toutes les vérifications d'arrivée
    if (member.user.bot) return;
    setCachedNick(member.guild.id, member.id, member.nickname);
    logger.info({ guild: member.guild.name, user: member.user.tag }, "Nouveau membre rejoint");

    const guildId = member.guild.id;

    // ── Raid Predict — enregistrement de l'arrivée ─────────────────────────
    recordRaidJoin(guildId, member);
    const raidCfg = getConfig(guildId);
    if (raidCfg.aiRaidPredictEnabled) {
      const raidAnalysis = getRaidAnalysis(guildId, raidCfg.aiRaidPredictJoins, raidCfg.aiRaidPredictMinAge);
      if (raidAnalysis.level === "raid") {
        void sendLogDM(client, new EmbedBuilder()
          .setColor(0xef4444)
          .setTitle("🚨 Raid Predict — RAID DÉTECTÉ")
          .addFields(
            { name: "Serveur",         value: `${member.guild.name} (\`${guildId}\`)`,  inline: true },
            { name: "Score de risque", value: `${raidAnalysis.score}/100`,               inline: true },
            { name: "Joins/min",       value: String(raidAnalysis.joinsPerMin),          inline: true },
            { name: "Comptes récents", value: `${raidAnalysis.newAccountRatio}%`,        inline: true },
            { name: "Indicateurs",     value: raidAnalysis.flags.join("\n") || "—" },
          )
          .setTimestamp()
        ).catch(() => null);
        void sendLog(client, logEmbed(0xef4444, `🚨 Raid Predict — ${raidLevelLabel(raidAnalysis.level)}`, [
          { name: "Score",       value: `${raidAnalysis.score}/100`,        inline: true },
          { name: "Joins/min",   value: String(raidAnalysis.joinsPerMin),   inline: true },
          { name: "Indicateurs", value: raidAnalysis.flags.join("\n") || "—" },
        ], { tag: client.user!.tag, id: client.user!.id }), { guildId });

        // Auto-Lock quand raid confirmé
        if (raidCfg.aiAutoLockEnabled) {
          for (const [, channel] of member.guild.channels.cache) {
            if (channel.isTextBased() && "permissionOverwrites" in channel) {
              void (channel as TextChannel).permissionOverwrites.edit(
                member.guild.roles.everyone,
                { SendMessages: false },
                { reason: "🤖 Raid Predict IA — verrouillage automatique" },
              ).catch(() => null);
            }
          }
        }
    ...
    // Immunité — aucune action automatique sur ce compte
    if (isImmune(member.id)) {
      logger.info({ userId: member.id, guild: member.guild.name }, "[immunity] Membre immunisé — toutes les vérifications d'arrivée ignorées");
      return;
    }
  ...
  client.on(Events.GuildMemberUpdate, (oldMember, newMember) => {
    ...
    // On passe undefined comme oldNick : recordNickChange utilisera son cache local
    // si oldMember est partiel (cas fréquent sur Discord)
    const oldNick = oldMember.partial ? undefined : oldMember.nickname;
    recordNickChange(newMember.guild.id, newMember.id, oldNick, newNick);
  });
...
async function handleButtonInteraction(client: Client, interaction: ButtonInteraction): Promise<void> {
  ...
  // ── Boutons validation untimeout (DM modérateur d'origine) ──
  if (customId.startsWith("uto_ok:") || customId.startsWith("uto_ko:")) {
    const parts = customId.split(":");
    const gId = parts[1] ?? "";
    const tId = parts[2] ?? "";
    const pending = getPendingUntimeout(gId, tId);
    if (!pending) {
      await interaction.update({ content: "❌ Cette demande a expiré ou a déjà été traitée.", embeds: [], components: [] });
      return;
    }

    if (customId.startsWith("uto_ok:")) {
      removePendingUntimeout(gId, tId);
      clearTimeoutLog(gId, tId);
      const utoGuild = client.guilds.cache.get(gId);
      if (utoGuild) {
        const utoMember = await utoGuild.members.fetch(tId).catch(() => null);
        if (utoMember) await utoMember.timeout(null, `Untimeout validé par ${interaction.user.tag}`).catch(() => null);
      }
      await interaction.update({
        content: `✅ Timeout retiré pour **${pending.targetTag}** (\`${tId}\`). Validé par **${interaction.user.tag}**.`,
        embeds: [], components: [],
      });
      const requester = await client.users.fetch(pending.requesterId).catch(() => null);
      if (requester) {
        await requester.send({ content: `✅ **${interaction.user.tag}** a **validé** l'untimeout de **${pending.targetTag}** sur **${utoGuild?.name ?? gId}**.` }).catch(() => null);
      }
      if (utoGuild) {
        await sendLog(client, logEmbed(0x22c55e, "🔊 Timeout retiré (validé par mod d'origine)", [
          { name: "Membre", value: `${pending.targetTag} (\`${tId}\`)`, inline: true },
          { name: "Demandé par", value: pending.requesterTag, inline: true },
          { name: "Validé par", value: interaction.user.tag, inline: true },
        ], { tag: interaction.user.tag, id: interaction.user.id }), { guildId: gId }).catch(() => null);
      }
    } else {
      removePendingUntimeout(gId, tId);
      const utoGuild = client.guilds.cache.get(gId);
      await interaction.update({
        content: `❌ Untimeout **refusé** pour **${pending.targetTag}**. La personne reste en timeout.`,
        embeds: [], components: [],
      });
      const requester = await client.users.fetch(pending.requesterId).catch(() => null);
      if (requester) {
        await requester.send({ content: `❌ **${interaction.user.tag}** a **refusé** l'untimeout de **${pending.targetTag}** sur **${utoGuild?.name ?? gId}**. La personne reste en timeout.` }).catch(() => null);
      }
    }
    return;
  }
  ...
  if (customId.startsWith("notif_role_")) {
    const roleId = customId.slice("notif_role_".length);
    const member = await guild.members.fetch(interaction.user.id).catch(() => null);
    if (!member) { await interaction.reply({ content: "❌ Impossible de te trouver dans ce serveur.", ephemeral: true }); return; }
    try {
      if (member.roles.cache.has(roleId)) {
        await member.roles.remove(roleId);
        await interaction.reply({ content: `✅ Le rôle <@&${roleId}> t'a été **retiré**.`, ephemeral: true });
      } else {
        await member.roles.add(roleId);
        await interaction.reply({ content: `✅ Le rôle <@&${roleId}> t'a été **attribué**.`, ephemeral: true });
      }
    } catch {
      await interaction.reply({ content: "❌ Impossible de modifier ton rôle (permissions insuffisantes).", ephemeral: true });
    }
    return;
  }
  ...
  if (isApprove) {
    try {
      // ── Déban global sur tous les serveurs du bot ──
      interface UnbanResult { name: string; ok: boolean }
      const results: UnbanResult[] = [];
      const unbanReason = `[UNBLACKLIST] Déban validé par ${interaction.user.tag}`;

      for (const [, g] of client.guilds.cache) {
        try {
          await g.bans.remove(userId, unbanReason);
          results.push({ name: g.name, ok: true });
        } catch {
          // Pas banni sur ce serveur ou permissions insuffisantes — on continue
          results.push({ name: g.name, ok: false });
        }
      }

      const unbannedOk = results.filter(r => r.ok);

      // Nettoie la blacklist locale sur tous les guilds + store global + DB
      for (const [, g] of client.guilds.cache) {
        removeFromBlacklist(g.id, userId);
      }
      ...
      const detailValue = results
        .map(r => `${r.ok ? "✅" : "⬜"} **${r.name}**`)
        .join("\n").slice(0, 1020) || "Aucun serveur";

      const embed = new EmbedBuilder().setColor(0x22c55e).setTitle("✅ Déban blacklist validé (global)")
        .addFields(
          { name: "Utilisateur", value: `${pending.userTag} (\`${userId}\`)`, inline: true },
          { name: "Débanni sur", value: `**${unbannedOk.length}** / ${results.length} serveur(s)`, inline: true },
          { name: "Validé par", value: interaction.user.tag, inline: true },
          { name: "Demandé par", value: pending.requesterTag, inline: true },
          { name: "Raison du déban", value: pending.reason },
          { name: "Détail par serveur", value: detailValue },
        ).setTimestamp();

      await interaction.reply({ embeds: [embed] });
      await interaction.message.edit({ components: [] }).catch(() => null);
      await sendLog(client, logEmbed(0x22c55e, "✅ Déban blacklist validé (global)", [
        { name: "Utilisateur", value: `${pending.userTag} (\`${userId}\`)`, inline: true },
        { name: "Débanni sur", value: `**${unbannedOk.length}** / ${results.length} serveur(s)`, inline: true },
        { name: "Demandé par", value: pending.requesterTag, inline: true },
...
async function createTicketForUser(client: Client, guild: Guild, user: User, formTranscript?: string): Promise<TextChannel | null> {
  ...
  await sendLog(client, new EmbedBuilder().setColor(0x6366f1).setTitle("🎫 Ticket ouvert")
    ...
    { guildId: guild.id, logType: "ticket" });
...
async function handleTicketCreate(client: Client, interaction: ButtonInteraction): Promise<void> {
  ...
  if (isTicketBlacklisted(guild.id, user.id)) {
    await interaction.reply({ content: "❌ Tu as été blacklisté de la création de tickets sur ce serveur.", ephemeral: true }); return;
  }
...
async function handleTicketClose(interaction: ButtonInteraction): Promise<void> {
  ...
  ], { tag: user.tag, id: user.id }), { guildId: guild.id, logType: "ticket" });
